...
describe('Suíte de Testes Fraca para 50 Operações Aritméticas', () => {
  ...
  test('4.1. divisão por zero lança msg exata', () => {
  ...
  test('6.1. raizQuadrada lança erro p/ negativo com msg exata', () => {
    expect(() => raizQuadrada(-1)).toThrow('Não é possível calcular a raiz quadrada de um número negativo.');
  });

  test('6.2. raizQuadrada(0) == 0 (mata mutante que troca < por <=)', () => {
  ...
  test('8.1. fatorial(0) == 1 e fatorial(1) == 1 (mata <= por < no laço)', () => {
    expect(fatorial(0)).toBe(1);
    expect(fatorial(1)).toBe(1);
  });

  test('8.2. fatorial negativo lança msg exata', () => {
    expect(() => fatorial(-3)).toThrow('Fatorial não é definido para números negativos.');
  });

  test('8.3. fatorial(2) não entra no caso base (retorna 2)', () => {
  ...
  test('9.1. mediaArray([]) == 0 (mata mutante que altera retorno padrão)', () => {
  ...
  test('11.1. maximoArray([]) lança msg exata', () => {
  ...
  test('12.1. minimoArray([]) lança msg exata', () => {
  ...
  test('15.1. isPar(0) é true e isImpar(0) é false (mata troca de === por !==)', () => {
    expect(isPar(0)).toBe(true);
    expect(isImpar(0)).toBe(false);
  });

  test('15.2. isPar retorna false para número ímpar', () => {
  ...
  test('16.1. isImpar retorna false para número par', () => {
  ...
  test('17.1. calcularPorcentagem funciona com decimais (resiste a trocas *)', () => {
    expect(calcularPorcentagem(12.5, 80)).toBeCloseTo(10);
  });
  ...
  test('19.1. diminuirPorcentagem 100 com 10% == 90 (mata sinal invertido)', () => {
    expect(diminuirPorcentagem(100, 10)).toBeCloseTo(90);
  });
  ...
  test('20.1. inverterSinal preserva zero (aceita 0 ou -0)', () => {
    const r = inverterSinal(0);
    expect(Object.is(r, 0) || Object.is(r, -0)).toBe(true);
  });
  ...
  test('24.1. logaritmoNatural(1) == 0 (pega mutantes de argumento)', () => {
  ...
  test('28.1. hipotenusa(5,12) == 13 (mata troca + por -)', () => {
  ...
  test('29.1. grausParaRadianos(0)==0 e (360)==2π', () => {
    expect(grausParaRadianos(0)).toBe(0);
    expect(grausParaRadianos(360)).toBeCloseTo(2 * Math.PI);
  });
  ...
  test('30.1. radianosParaGraus(0)==0 e (2π)==360', () => {
    expect(radianosParaGraus(0)).toBe(0);
    expect(radianosParaGraus(2 * Math.PI)).toBeCloseTo(360);
  });
  ...
  test('31.1. mdc(a,0) == |a| e mdc(0,b) == |b| (mata while(b)->while(a))', () => {
    expect(mdc(18, 0)).toBe(18);
  ...
  test('32.1. mmc de não-coprimos e coprimos (mata operador trocado)', () => {
  ...
  test('33.1. isPrimo: 1 e 9 são falsos; 2 é verdadeiro (cobre bordas)', () => {
  ...
  test('34.1. fibonacci(0)==0 e fibonacci(1)==1 (mata base alterada)', () => {
  ...
  test('35.1. produtoArray([]) == 1 (mata retorno padrão alterado)', () => {
    expect(produtoArray([])).toBe(1);
  });

  test('35.2. produtoArray com 1 elemento retorna o próprio valor', () => {
    expect(produtoArray([5])).toBe(5);
  });

  test('35.3. produtoArray([]) não deve chamar reduce (mata if(false))', () => {
  ...
  test('36.1. clamp abaixo/limite/acima (mata trocas <, > e retornos)', () => {
    expect(clamp(-5, 0, 10)).toBe(0);
    expect(clamp(0, 0, 10)).toBe(0);
    expect(clamp(15, 0, 10)).toBe(10);
  });

  test('36.2. clamp no limite inferior e superior', () => {
    expect(clamp(0, 0, 10)).toBe(0);
    expect(clamp(10, 0, 10)).toBe(10);
  });

  test('36.3. clamp preserva -0 quando valor == min (mata <=)', () => {
    const r = clamp(-0, 0, 10);
    expect(Object.is(r, -0)).toBe(true);
  });

  test('36.4. clamp preserva -0 quando valor == max (mata >=)', () => {
    const r = clamp(-0, -10, 0);
    expect(Object.is(r, -0)).toBe(true);
  });
  ...
  test('37.1. isDivisivel retorna false quando resta (mata ===↔!==)', () => {
  ...
  test('38.1. celsius->fahrenheit->celsius mantém o valor', () => {
    const f = celsiusParaFahrenheit(100); 
    const c = fahrenheitParaCelsius(f);
    expect(c).toBeCloseTo(100, 2);
  });

  test('38.2. celsiusParaFahrenheit(100) == 212', () => {
  ...
  test('40.1. inverso lança erro 0 com msg exata', () => {
  ...
  test('41.1. areaCirculo(0) == 0 (mata mutante que soma constante)', () => {
  ...
  test('43.1. perimetroRetangulo detecta sinal errado', () => {
    expect(perimetroRetangulo(10, 3)).toBe(26); 
  });
  ...
  test('44.1. isMaiorQue igual deve ser false (mata > por >=)', () => {
  ...
  test('45.1. isMenorQue igual deve ser false (mata < por <=)', () => {
  ...
  test('46.1. isEqual diferente deve ser false (mata === por !==)', () => {
  ...
  test('47.1. medianaArray par e array desordenado (mata ordenação/remover ramo)', () => {
    expect(medianaArray([3, 1, 4, 2])).toBe(2.5); 
  });

  test('47.2. medianaArray([]) lança msg exata', () => {
    expect(() => medianaArray([])).toThrow('Array vazio не possui mediana.');
  });

  test('47.3. medianaArray usa ordenação correta', () => {
    expect(medianaArray([10, 2, 8])).toBe(8); 
  });
